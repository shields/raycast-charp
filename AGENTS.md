...
make leipzig        # refresh src/leipzig-freq.json from Leipzig corpora (~0.5GB download)
...
- `src/characters.ts`, `src/characters.json`, and `src/leipzig-freq.json` are
  generated — edit the generation scripts (`generate-data.ts`,
  `compute-leipzig-freq.ts`) instead
